...
export class PaisPage implements OnInit {
    ...
    async alertEliminar(pai_id: number, pai_nombre: string) {
        const alert = await this.alertCtrl.create({
            header: 'País',
            subHeader: 'Eliminar',
            message: '¿Estás seguro de eliminar el país ' + pai_nombre + '?',
    ...
    async eliminar(pai_id: number, pai_nombre: string) {
        try {
            await this.paisService.eliminar(pai_id).subscribe(
                response => {
                    this.alertEliminado(pai_id, 'El país: ' + pai_nombre + ' ha sido eliminado');
                },
                error => {
                    console.error('Error:', error);
                    if (error.response?.status == 204) {
                        this.alertEliminado(pai_id, 'El país: ' + pai_nombre + ' no se puede eliminar');
                    }
                    if (error.response?.status == 500) {
                        this.alertEliminado(pai_id, 'El país: ' + pai_nombre + ' no se puede eliminar esta siendo utilizado por otro registro');
                    }
                    if (error.response?.status == 401) {
                        this.alertEliminado(pai_id, 'El país: ' + pai_nombre + ' no se puede eliminar por que no tiene las credenciales');
                    }
                }
            );
        } catch (error) {
