...
export class Ventas {
  ...
  detalle(ven_id: string | null = '', extra: string = ''): Observable<any> {
    const url = `${this.url}/` + ven_id + extra;
    return new Observable(observer => {
      axios.get(url, {
        withCredentials: true,
        headers: this.headers
  ...
  actualizar(ven_id: number, venta: any): Observable<any> {
    const url = `${this.url}/${ven_id}`;
    return new Observable(observer => {
      axios.put(url, venta, {
        withCredentials: true,
        headers: this.headers
  ...
  eliminar(ven_id: number): Observable<any> {
    const url = `${this.url}/${ven_id}`;
    return new Observable(observer => {
      axios.delete(url, {
        withCredentials: true,
